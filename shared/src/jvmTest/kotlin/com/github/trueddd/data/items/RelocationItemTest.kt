...
class RelocationItemTest : EventGateTest() {
    ...
    fun `relocate backward`() = runTest {
        ...
        val genreBack = genreDistribution.genreAtPosition(5)
    ...
    fun `relocate forward`() = runTest {
        ...
        handleAction(ItemReceive(user, item))
        val genreBack = genreDistribution.genreAtPosition(6)
        handleAction(ItemUse(user, item.uid, listOf(genreBack.ordinal.toString())))
    ...
    fun `relocate forward when both directions are possible`() = runTest {
        val superposition = genreDistribution.genres.let { genres ->
        ...
        assertEquals(
            genreDistribution.genreAtPosition(positionOf(user) - 2),
            genreDistribution.genreAtPosition(positionOf(user) + 2)
        )
        handleAction(ItemReceive(user, item))
        val genre = genreDistribution.genreAtPosition(positionOf(user) - 2)
